...
% 1) Функция получает данные книги ордеров из последнего файла в дириктории
% с архивом стаканов
%
% 2) просчитывает на N ходов K раз случайные сделки на примере открытых стаканов,
% что трейдер меняет ход и закрывает ордер по рынку
%
% 3) Считает вероятность оказаться цене по паре на конкретной позиции
% Возвращает по каждой паре вектор веорятностей через N ходов изминиться на % от настоящей позиции
% от 0:200 где 100 это остаться на месте
...
% формирует массив order_sum в которой 1 поле имя тикера
% 2 накопительный стакан asks 
% 3 накопительный стакан bids 
